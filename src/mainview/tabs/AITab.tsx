    ...
    async function sendMessage(question?: string) {
        ...
        try {
            ...
            if (queryAIFn) {
                ...
                response = "AI responses require the Electrobun runtime and a configured API key.";
            }
    ...
    return (
        <div className="h-full flex flex-col relative overflow-hidden bg-white dark:bg-black transition-colors duration-500 font-sans">
            ...
            <div className="relative z-10 flex-1 flex flex-col items-center pt-24">
                ...
                {/* Landing Headlines */}
                <div className={`text-center px-6 transition-all duration-700 ${hasMessages ? 'opacity-0 -translate-y-8 absolute pointer-events-none' : 'opacity-100 translate-y-0'}`}>
                    <h1 className="text-4xl md:text-6xl text-black dark:text-white leading-tight font-playfair tracking-normal">
                        How can I <span className="italic font-playfair">help</span> you?
                    </h1>
                ...
                <div className={`w-full max-w-4xl mx-auto flex-1 flex flex-col transition-all duration-700 ${hasMessages ? 'mt-0 rounded-none border-none bg-transparent shadow-none' : 'mt-16 bg-white/40 dark:bg-black/40 backdrop-blur-2xl border-t border-x border-black/5 dark:border-white/10 rounded-t-[2.5rem] shadow-2xl relative px-6 py-6'}`}>
                                ...
                                {messages.map((msg) => (
                                    <div key={msg.id} className={`flex ${msg.role === "user" ? "justify-end" : "justify-start"}`}>
                                        <div
                                            ...
                                            `}
                                        >
                                            <pre className="whitespace-pre-wrap font-sans m-0 break-words">{msg.content}</pre>
                                        </div>
                                    </div>
                                ))}
                                {loading && (
                                    <div className="flex justify-start">
                                        <div className="bg-white dark:bg-[#0A0A0A] rounded-3xl rounded-bl-md px-6 py-5 shadow-sm border border-black/5 dark:border-white/10">
                                            <div className="flex gap-2">
                                                <div className="w-2 h-2 rounded-full bg-black/40 dark:bg-white/40 animate-bounce" style={{ animationDelay: "0ms" }} />
                                                <div className="w-2 h-2 rounded-full bg-black/40 dark:bg-white/40 animate-bounce" style={{ animationDelay: "150ms" }} />
                                                <div className="w-2 h-2 rounded-full bg-black/40 dark:bg-white/40 animate-bounce" style={{ animationDelay: "300ms" }} />
                                            </div>
                                        </div>
                                    </div>
                                )}
                    ...
                    <div className={`${hasMessages ? 'absolute bottom-6 left-0 right-0 px-6' : 'w-full'} z-20 transition-all duration-700`}>
                        <form onSubmit={(e) => { e.preventDefault(); sendMessage(); }} className="max-w-3xl mx-auto mb-8 relative">
                            <div className="bg-white dark:bg-black rounded-2xl shadow-[0_8px_30px_rgba(0,0,0,0.06)] dark:shadow-[0_8px_30px_rgba(255,255,255,0.03)] border border-black/5 dark:border-white/10 p-4">
                                ...
                                <div className="flex items-center justify-between pt-1">
                                    <div className="flex items-center gap-4 text-black/40 dark:text-white/40">
                                        <button type="button" className="hover:text-black/70 dark:hover:text-white/70 transition-colors p-1">
                                            <Plus className="w-5 h-5" />
                                        </button>
                                    </div>

                                    <div className="flex items-center gap-3">
                                        <button type="button" className="text-black/40 dark:text-white/40 hover:text-black/70 dark:hover:text-white/70 transition-colors">
